...
import ru.bz.baltic_shipyard_inventory_service.applications.usecases.warehouse.lot.LotUseCases
import ru.bz.baltic_shipyard_inventory_service.applications.usecases.warehouse.stock.InventoryUseCases
import ru.bz.baltic_shipyard_inventory_service.applications.usecases.warehouse.warehouse.WarehouseUseCases
...
@Service
class ValidateTransferItemByLocationUseCase(
    ...
    private fun warehouseViolationToBuilder (
        ...
        if(!warehouseUseCases.warehouseExists(transfer.warehouseCode)) {
            return  builder.warehouseCodeViolation(
                Violation(
                    ...
                    externalDescription = messageSource.getMessage(
        ...
        if(!warehouseUseCases.binOfWarehouseExists(
                SearchLocationDetailShotFilter(
                    transfer.warehouseCode,
                    transfer.binCodeSource
                )
            )){
